    ...
    , allocDirty("%s:allocDirty", name)
{
  ...
  tmpsection = section; // copying the section variable
    ...
    //my code
    invdb = new std::multiset<uint32_t>(); 
...
void SMPCache::doRead(MemRequest *mreq)
{
   PAddr addr = mreq->getPAddr();
    ...
    if (l && l->canBeRead()) {
        ...
        mreq->goUp(hitDelay);
//my code start
	if (std::find(cap.begin(), cap.end(), tag) == cap.end()) { 
		if (cap.size() == cache->getNumLines()) { 
                     
			cap.erase(cap.begin());
		} 
	} else {
			cap.erase(std::find(cap.begin(),cap.end(),tag));
		}
 cap.push_back(tag); 
//my code end
        return;
...
rcoheMiss.inc();
} else if (std::find(comp.begin(), comp.end(), tag) == comp.end()) {
rcompMiss.inc();  //comp miss
comp.push_back(tag);
} else {
	rreplMiss.inc(); //capmiss+confmiss	
    }		
...
if (invdb->find(tag)!=invdb->end() || (l && l->getState()==DMESI_SHARED)){
wcoheMiss.inc();
} else if (std::find(comp.begin(), comp.end(), tag) == comp.end()) {
wcompMiss.inc();  //comp miss
comp.push_back(tag);
} else {
	wreplMiss.inc(); //capmiss+confmiss	
    }		
...
//my code
	    ...
	    invdb->insert(tag); //add invalidated tag before invalidation

            l->invalidate();
	    //invdb->insert(l->prevTag);
        }
