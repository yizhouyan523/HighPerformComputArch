...
*CacheAssoc<State, Addr_t, Energy>::findLine2Replace(Addr_t addr, bool ignoreLocked)
{
    ...
    {
        Line **l = setEnd -1;
		Line **oldest_free = setEnd; // any number > setEnd - 1 is fine
        ...
        while(l >= theSet) {
			...
			else if (oldest_free == lineFree && policy == NXLRU && !(*l)->isLocked())
				lineFree = l;
    ...
    if (lineFree == 0) {
        ...
        if (policy == RANDOM) {
			...
			I(policy == NXLRU);
			lineFree = setEnd-2; // Get the second oldest line possible
        } else {
            I(policy == LRU);
